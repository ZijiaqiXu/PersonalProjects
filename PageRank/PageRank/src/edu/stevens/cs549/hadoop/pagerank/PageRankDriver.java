...
public class PageRankDriver {
	...
	static void diff(String input1, String input2, String output, int reducers)
		...
		// delete tempdiff directory if exists
	...
	static void joinName(String input1, String input2, String output, int reducers)
			throws Exception {
		System.out.println("Join name Job Started");
		Job job = Job.getInstance(); // Creates a new Job
		job.setJarByClass(PageRankDriver.class); // Sets the Driver class
		job.setNumReduceTasks(reducers); // Sets the number of reducers

		FileInputFormat.addInputPath(job, new Path(input1)); // Adds input and output paths
		FileInputFormat.addInputPath(job, new Path(input2));
		// delete output directory if exists
		deleteDirectory(output);
		FileOutputFormat.setOutputPath(job, new Path(output));

		job.setMapperClass(EquiJoinMapper.class); // Sets Mapper and Reducer Classes
		...
		// Prints message on successful completion or error.
		System.out.println(job.waitForCompletion(true) ? "Join name Job Completed" : "Join name Job Error");
		// Exits once job finishes.
		deleteDirectory(input1); // Deletes the input directory

	}
	
	public static void composite(String input, String output, String interim1,
			String interim2, String diff, String pagename,int reducers) throws Exception {
		/*
		 * TODO 
		 */
		System.out.println("Zijiaqi Xu (10456340)");
		int counter = 0;
		...
		while (difference >= 30) // continues operation till difference is of specified value
		{
			if (i % 2 == 0) {
			...
			if (i % 3 == 0) // for every 3 iterations calculates the difference
			{
				 ...
				 */
				difference = readDiffResult(diff); // Reads difference from diff
				System.out.println("Difference updates to:" + difference);
				deleteDirectory(diff); // deletes diff
			}
						...
						// is the input directory
		{
			...
			counter++;
			finish(interim2, interim1, reducers);
			joinName(interim1, pagename, output, reducers);
			summarizeResult(output);
		} else // for even i, interim1 is the input directory
		{
			deleteDirectory(interim2); // Deletes other directory
			counter++;
			finish(interim1, interim2, reducers);
			joinName(interim2, pagename, output, reducers);
			summarizeResult(output);
		}
		System.out.println();
	...
	static void summarizeResult(String path) throws Exception {
		...
		if (fs.exists(finpath)) {
			...
			try {
				// TODO modify this to change node id to page names
