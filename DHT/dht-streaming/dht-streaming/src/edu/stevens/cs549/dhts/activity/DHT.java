...
//		routing.joinCheck();
	 ...
	 */
	protected String[] get(NodeInfo n, String k) throws Failed, Invalid {
	 ...
	 */
	public void add(NodeInfo n, String k, String v) throws Failed, Invalid {
		if (n.addr.equals(info.addr)) {
			try {
				add(k, v);
			} catch (Invalid e) {
	 ...
	 */
	public void delete(NodeInfo n, String k, String v) throws Failed, Invalid {
		if (n.addr.equals(info.addr)) {
			try {
				delete(k, v);
			} catch (Invalid e) {
	 ...
	 */
	public void addNet(String skey, String v) throws Failed, Invalid {
		int id = NodeKey(skey);
	 ...
	 */
	public String[] getNet(String skey) throws Failed, Invalid {
		int id = NodeKey(skey);
	 ...
	 */
	public void deleteNet(String skey, String v) throws Failed, Invalid {
		int id = NodeKey(skey);
	 ...
	 */
	public void join(String uri) throws Failed, Invalid, URISyntaxException {
		setPred(null);
		NodeInfo info = getNodeInfo();
		NodeInfo succ;
		/*
		 ...
		 * from us.
		 */
		succ = client.findSuccessor(new URI(uri), info.id);
		setSucc(succ);
	...
	public EventOutput listenForBindings(int id, String key) {
		// create event output stream and add to broadcaster
		// and register it in the state object
		final EventOutput eventOutput = new EventOutput();
		debug("listforBindings("+key+")");
		state.addListener(id, key, eventOutput);
		return eventOutput;
	}
	
	// Web method
	public void stopListening(int id, String key) {
		// remove event output stream from broadcaster
		debug("stopListening("+key+")");
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		 ...
		 * where those bindings are stored.
		 */
		int id = NodeKey(key);
		NodeInfo node;
		//if the key's id is equal to the local node
		if (info.id == id) {
			node = info;
		}
		else {
		...
		// web service call to dest node 
		EventSource eventSource = client.listenForBindings(node, info.id, key);
		// register eventListener
		eventSource.register(listener, IDHTNode.NEW_BINDING_EVENT);
		//register eventSource in the state object
		state.addCallback(key, eventSource);
		//open stream
		eventSource.open();
		if (eventSource.isOpen()) {System.out.println("eventSource opened succesfully");}
		else {System.out.println("eventSource open failed");}
		
	}
	
	public void listenOff(String key) throws DHTBase.Failed {
		/*
		 * Stop listening for new binding events for this key.  Need to
