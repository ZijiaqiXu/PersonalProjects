...
public class NodeService {
	...
	public Response getBinding(String k) {
		...
		info("get()");
		try {
			TableRow r = new TableRow(k, dht.get(k));
			return response(r);
		} catch (Invalid i) {
			return responseNull();
		}
	...
	public Response closestPrecedingFinger(int id) {
		advanceTime();
		info("getCloesetPrecedingFinger()");
	...
	public Response listenForBindings(int id, String k) {
		advanceTime();
		info("listenForBindings()");
		return response(dht.listenForBindings(id, k));
	}
	
	public Response stopListening(int id, String k) {
		advanceTime();
		info("stopListening()");
		dht.stopListening(id, k);
		return response();
	}
