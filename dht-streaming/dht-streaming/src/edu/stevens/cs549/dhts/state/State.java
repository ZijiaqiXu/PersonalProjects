...
public class State implements IState, IRouting {
	...
	public synchronized NodeInfo closestPrecedingFinger(int id) {
		 ...
		 * node. See DHTBase.inInterval()
		 */
		NodeInfo finger = info;
		for (int i = NKEYS - 1; i >= 0; i--) {
			if (DHTBase.inInterval(getFinger(i).id, info.id, id, false)) {
				return finger = getFinger(i);
			}
		}
		return finger;
	}
	...
	public void addListener(int id, String key, EventOutput os) {
		// if a sseBroadcaster for the key already exists on server-side, 
		// use the existing one
		SseBroadcaster broadcaster;
		if (listeners.containsKey(key)) {
			broadcaster = listeners.get(key);
		} else {
			// initialize an new broadcaster and register it in the listeners map
			broadcaster = new SseBroadcaster();
			listeners.put(key, broadcaster);
		}
		// add eventOutput to the broadcaster
		broadcaster.add(os);
		
		// register the key-output with the client node ID for closing output stream
		Map<String, EventOutput> map;
		// if the map already exists
		if (outputs.containsKey(id)) {
			// put the new key-output pair with the client ID in the map
			map = outputs.get(id);
			map.put(key, os);
		} else {
			// initialize new map and register it
			map = new HashMap<>();
			outputs.put(id, map);
			// put the new key-output pair in
			map.put(key, os);
		}
	}
	
	public void removeListener(int id, String key) {
		// Close the event output stream.
		if (outputs.containsKey(id)) {
			Map<String, EventOutput> map = outputs.get(id);
			if (map.containsKey(key)) {
				// if outputs contains any output under this node id with the key
				EventOutput os = map.get(key);
	...
	private void broadcastAddition(String key, String value) {
		// broadcast an added binding (use IDHTNode.NEW_BINDING_EVENT for event name).
		if (listeners.containsKey(key)) {
