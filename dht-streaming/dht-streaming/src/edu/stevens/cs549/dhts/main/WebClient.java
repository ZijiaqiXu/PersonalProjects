...
public class WebClient {
	...
	public void add(NodeInfo node, String k, String v) throws DHTBase.Failed, DHTBase.Invalid {
		URI addPath = UriBuilder.fromUri(node.addr)
				.queryParam("key", k)
				.queryParam("val", v)
				.build();
	...
	public TableRow get(NodeInfo node, String k) throws DHTBase.Failed, DHTBase.Invalid {
		URI getPath = UriBuilder.fromUri(node.addr)
				.queryParam("key", k)
				.build();
	...
	public void delete(NodeInfo node, String k, String v) throws DHTBase.Failed, DHTBase.Invalid {
		URI delPath = UriBuilder.fromUri(node.addr)
				.queryParam("key", k)
				.queryParam("val", v)
				.build();
	...
	public NodeInfo getSucc(NodeInfo node) throws DHTBase.Failed {
		URI succPath = UriBuilder.fromUri(node.addr)
				.path("succ")
				.build();
	...
	public NodeInfo getPred(NodeInfo node) throws DHTBase.Failed {
		URI predPath = UriBuilder.fromUri(node.addr)
				.path("pred")
				.build();
	...
	public NodeInfo getClosestPrecedingFinger(NodeInfo node, int id) throws DHTBase.Failed {
		URI closestPreFingerPath = UriBuilder.fromUri(node.addr)
				.path("finger")
				.queryParam("id", id)
				.build();
	...
	public NodeInfo findSuccessor(URI addr, int id) throws DHTBase.Failed {
		URI findSuccPath = UriBuilder.fromUri(addr)
				.path("find")
				.queryParam("id", id)
				.build();
	...
	public EventSource listenForBindings(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		// listen for SSE subscription requests on http://.../dht/listen?id=<id>&key=<key>
		...
		//build web target
		URI listenOnPath = UriBuilder.fromUri(node.addr)
				.path("listen")
				.queryParam("id", id)
				.queryParam("key", skey)
				.build();
		WebTarget target = listenClient.target(listenOnPath);
		//build eventSource
		return EventSource.target(target).build();
	}

	public void listenOff(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		// listen for SSE subscription requests on http://.../dht/listen?key=<key>
		// On the service side, don't expect LT request or response headers for this request.
		URI listenOffPath = UriBuilder.fromUri(node.addr)
				.path("listen")
				.queryParam("id", id)
				.queryParam("key", skey)
				.build();
		info("client listenOff(" + listenOffPath + ")");
		Response response = deleteRequest(listenOffPath);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("DELETE /listen?id="+ id + "&key=" + skey);
		} else {
			log.info("Ok: DELETE /listen?id=" + id + "&key=" + skey);
		}
	}
